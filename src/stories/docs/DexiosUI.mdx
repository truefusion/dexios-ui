...
For secondary text with a plain background we recommend a shade of 500.

#### Sections

When dividing content into sections, pick a large gap size.
Initial gap size should match initial padding.
Nested sections should contain only components that are related to each other.
For every level you dive, divide the gap size by half.
If you run out of sizes, you probably nested too deeply or didn't start off with a large enough size.
